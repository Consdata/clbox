export const feedbackStatsFactory = (
...
) => functions.firestore.document('team/{team}/inbox/{chapterLeader}/message/{messageId}').onCreate( // S10
  async (change, context) => {
    ...
    const monthRef = firestore.collection(`team/${context.params.team}/stats`).doc(month); // S11

    await firestore.runTransaction(async trn => {
      const monthDoc = await trn.get(monthRef);
      const statsData = monthDoc.data();
      await trn.set(monthRef, { // S12
